#!/bin/sh
# planning-context: Stop-hook dispatcher for the v3 completion gate.
#
...
if [ ! -f "$TARGET" ] && [ -n "${HOME:-}" ]; then
    ...
    TARGET=$(ls "${HOME}/.claude/skills/planning-context/scripts/check-complete.sh" \
                "${HOME}/.claude/plugins/marketplaces/planning-context/scripts/check-complete.sh" \
                2>/dev/null | head -1)
fi
