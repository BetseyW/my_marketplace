---
description: "Run a planning-aware cadence with Claude Code's /loop. Default tick checks plan status, runs check-complete, nudges progress.md and handoff.md updates if stalled."
...
3. Compose the loop prompt:
   ...
   - Else: use the default planning tick prompt:
     ```
     Read task_plan.md, progress.md, and handoff.md. Run scripts/check-complete.sh to see remaining phases.
