...
#     4. the block counter (<plan-dir>/.stop_blocks) is below cap (PCX_GATE_CAP, default 20)
...
if [ ! -f "$PLAN_FILE" ]; then
    echo "[planning-context] No task_plan.md found — no active planning session."
    exit 0
fi
...
# handoff_advisory: emit reminders about handoff.md state at end of turn.
# Missing → prompt to create it. Present but older than progress.md → prompt to
# refresh (progress advanced but the resume-snapshot didn't). Silent if fresh.
# Portable mtime via stat -c/-f, then date -r, then 0 (skip staleness check).
handoff_advisory() {
    HO_FILE="${PLAN_DIR}/handoff.md"
    PR_FILE="${PLAN_DIR}/progress.md"
    if [ ! -f "$HO_FILE" ]; then
        echo "[planning-context] Missing handoff.md — create it when preparing a handoff or at session end (resume-snapshot for next agent)."
        return 0
    fi
    _mt_ho=$(stat -c '%Y' "$HO_FILE" 2>/dev/null || stat -f '%m' "$HO_FILE" 2>/dev/null || date -r "$HO_FILE" +%s 2>/dev/null || echo 0)
    _mt_pr=$(stat -c '%Y' "$PR_FILE" 2>/dev/null || stat -f '%m' "$PR_FILE" 2>/dev/null || date -r "$PR_FILE" +%s 2>/dev/null || echo 0)
    if [ "$_mt_pr" -gt 0 ] && [ "$_mt_ho" -gt 0 ] && [ "$_mt_pr" -gt "$_mt_ho" ]; then
        echo "[planning-context] handoff.md is older than progress.md — refresh the snapshot before stopping so the resume entry is current."
    fi
}
...
advisory_report() {
    if [ "$COMPLETE" -eq "$TOTAL" ] && [ "$TOTAL" -gt 0 ]; then
        ...
        if [ "$IN_PROGRESS" -gt 0 ]; then
            echo "[planning-context] $IN_PROGRESS phase(s) still in progress."
        fi
        if [ "$PENDING" -gt 0 ]; then
            echo "[planning-context] $PENDING phase(s) pending."
        fi
    fi
    handoff_advisory
}
...
if [ "${BLOCKS}" -ge "${CAP}" ]; then
    advisory_report
    echo "[planning-context] gate cap reached ($BLOCKS/$CAP) — allowing stop."
    exit 0
fi
...
if [ "${BLOCKS}" -gt 0 ] && [ "${LEDGER_NOW}" -eq "${LEDGER_PREV}" ]; then
    advisory_report
    echo "[planning-context] no progress since last gate block — allowing stop."
    exit 0
fi
...
printf '{"decision":"block","reason":"[planning-context] Gated plan incomplete: phase '\''%s'\'' is in_progress (%s/%s complete, gate block %s/%s). Finish or update the plan, then stop."}\n' \
